...
| **[/train/](https://akusento.com/train/)** | An interactive drill suite with four modes: **Sentences** (mark accented morae in real Japanese text, Easy/Medium/Hard/Mixed difficulty), **Words** (single-word drills filterable by frequency range and word type, or your own pasted word list), **Homophones** (pairs that share a reading but differ in pitch, e.g. 箸/橋), and **Review mistakes** (auto-saved, browser-local review of previously missed answers). |
| **[/search/](https://akusento.com/search/)** | A pitch accent dictionary covering a lot of unique Japanese words and phrases, including wildcard and regex-style pattern search. |
